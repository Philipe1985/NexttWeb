...
$(document).ready(function () {
    $("#wizard").aiiaWizard({
        ...
        onButtonNextClick: function () {
            ...
            console.log()
            if (!$("#wizard").aiiaWizard('isFinalElementShown')) {
    ...
    document.querySelector("button.dirseg").onclick = function () {
        mover(document.querySelector("ul.esqseg"),
            document.querySelector("ul.dirseg"));
    };

    document.querySelector("button.esqseg").onclick = function () {
        mover(document.querySelector("ul.dirseg"),
            document.querySelector("ul.esqseg"));
    };

    $(document).on('keyup', '.filtroSearch', function (e) {
        var search = $.trim($(this).val());
        var idSel = $(this).attr('id').replace('txtBusca', '#filtroSelecionado')
        var idOpc = $(this).attr('id').replace('txtBusca', '#filtroOpcoes')
        search = search.toLowerCase();
        var hltd = $(idOpc+' li').each(function () {
        ...
        var dltd = $(idSel + ' li').each(function () {
            var refFiltro = $(this).attr('data-filtro').toLowerCase();
            if (refFiltro.indexOf(search) === -1) {
                $(this).addClass('oculto');
            }
    ...
    $(function () {
        $("#filtroOpcoesAreas, #filtroSelecionadoAreas").sortable({
            connectWith: ".connectedSortable"
        }).disableSelection();

        $("#filtroOpcoesSegmentos, #filtroSelecionadoSegmentos").sortable({
            connectWith: ".connectedSortable"
        }).disableSelection();
    });
});
...
function finalizarCadastro() {
    ...
    var jc = $.confirm({
        ...
        title: 'Aguarde!',
        content: 'Estamos armazenando as informações!',
        closeIcon: false,
        type: 'orange',
        animation: 'scale',
        buttons: {
        ...
        onDestroy: function () {
            ...
            jc2 = $.confirm({
                ...
                content: 'Aguarde! Você será redirecionado ao sistema em alguns segundos',
                icon: 'fa fa-check',
                theme: 'modern',
                closeIcon: false,
                type: 'green',
                ...
                onDestroy: function () {
                    window.location = "../relatorio/dashboard.cshtml";
                }
            });
        },
    });
...
; (function ($, window, document, undefined) {
    ...
    function getStepTitleTemplate(i, $step, settings) {
        ...
        var $newStepTitle = $("" +
            "<div class='aiia-wizard-step-title'>" +
            "<div class='col-md-12'>" +
