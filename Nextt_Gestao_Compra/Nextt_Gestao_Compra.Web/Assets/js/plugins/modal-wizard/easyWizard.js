...
$.fn.wizard = function (config) {
    ...
    var btns = "";
    btns += '<button type="button" class="btn btn-info wizard-button-exit"' + contentForModal + '>' + exitText + '</button>';
    btns += '<button type="button" class="btn btn-info wizard-button-back">' + backText + '</button>';
    btns += '<button type="button" class="btn btn-info wizard-button-next">' + nextText + '</button>';
    btns += '<button type="button" class="btn btn-primary wizard-button-finish">' + finishText + '</button>';
    ...
    btnNext.on("click", function () {
        ...
        step++;
        steps.hide();
        $(steps[step - 1]).fadeIn(1000);
        if (step === 3) {

            tabelaPack.columns.adjust().draw();
            tabelaPack.fixedHeader.adjust();
        } 
    ...
    btnFinish.on("click", function () {
        if (!validateFinish(step, steps[step - 1])) {
            ...
            $('#modalCadastroCompra').modal('hide');
        };
